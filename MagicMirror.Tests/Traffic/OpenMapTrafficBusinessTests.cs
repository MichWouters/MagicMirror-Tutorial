...
{
    public class OpenMapTrafficBusinessTests
    {
        ...
        // Mock objects
        private Mock<IOpenMapTrafficRepo> mockRepo;

        public OpenMapTrafficBusinessTests()
        {
            mockRepo = new Mock<IOpenMapTrafficRepo>();
            ...
            _service = new OpenMapTrafficService(mockRepo.Object, mapper);
        }
        ...
        public async Task Repo_GetEntity_Called_Once()
        {
            ...
            // Act
            await _service.GetTrafficModelAsync(Origin, Destination);

            // Assert
            mockRepo.Verify(x => 
                    x.GetTrafficInfoAsync(It.IsAny<string>(), It.IsAny<string>()),
                Times.Once);
        }

        private OpenMapTrafficEntity GetMockEntity()
        {
            return new OpenMapTrafficEntity
            {
                Info = new Info
                {
                    Statuscode = 0
